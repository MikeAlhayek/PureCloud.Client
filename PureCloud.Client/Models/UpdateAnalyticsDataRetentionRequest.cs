using System.Runtime.Serialization;
using System.Text;
using System.Text.Json.Serialization;

namespace PureCloudPlatform.Client.V2.Model;

/// <summary>
/// UpdateAnalyticsDataRetentionRequest
/// </summary>
[DataContract]
public partial class UpdateAnalyticsDataRetentionRequest : IEquatable<UpdateAnalyticsDataRetentionRequest>
{

    /// <summary>
    /// Initializes a new instance of the <see cref="UpdateAnalyticsDataRetentionRequest" /> class.
    /// </summary>
    [JsonConstructorAttribute]
    protected UpdateAnalyticsDataRetentionRequest() { }
    /// <summary>
    /// Initializes a new instance of the <see cref="UpdateAnalyticsDataRetentionRequest" /> class.
    /// </summary>
    /// <param name="RetentionDays">Analytics data retention period in days to set for the organization. (required).</param>
    public UpdateAnalyticsDataRetentionRequest(int? RetentionDays = null)
    {
        this.RetentionDays = RetentionDays;

    }



    /// <summary>
    /// Analytics data retention period in days to set for the organization.
    /// </summary>
    /// <value>Analytics data retention period in days to set for the organization.</value>
    [DataMember(Name = "retentionDays", EmitDefaultValue = false)]
    public int? RetentionDays { get; set; }


    /// <summary>
    /// Returns the string presentation of the object
    /// </summary>
    /// <returns>String presentation of the object</returns>
    public override string ToString()
    {
        var sb = new StringBuilder();
        sb.Append("class UpdateAnalyticsDataRetentionRequest {\n");

        sb.Append("  RetentionDays: ").Append(RetentionDays).Append("\n");
        sb.Append("}\n");
        return sb.ToString();
    }


    /// <summary>
    /// Returns true if objects are equal
    /// </summary>
    /// <param name="obj">Object to be compared</param>
    /// <returns>Boolean</returns>
    public override bool Equals(object obj)
    {
        // credit: http://stackoverflow.com/a/10454552/677735
        return this.Equals(obj as UpdateAnalyticsDataRetentionRequest);
    }

    /// <summary>
    /// Returns true if UpdateAnalyticsDataRetentionRequest instances are equal
    /// </summary>
    /// <param name="other">Instance of UpdateAnalyticsDataRetentionRequest to be compared</param>
    /// <returns>Boolean</returns>
    public bool Equals(UpdateAnalyticsDataRetentionRequest other)
    {
        // credit: http://stackoverflow.com/a/10454552/677735
        if (other == null)
        {
            return false;
        }

        return true &&
            (
                this.RetentionDays == other.RetentionDays ||
                this.RetentionDays != null &&
                this.RetentionDays.Equals(other.RetentionDays)
            );
    }

    /// <summary>
    /// Gets the hash code
    /// </summary>
    /// <returns>Hash code</returns>
    public override int GetHashCode()
    {
        // credit: http://stackoverflow.com/a/263416/677735
        unchecked // Overflow is fine, just wrap
        {
            int hash = 41;
            // Suitable nullity checks etc, of course :)
            if (this.RetentionDays != null)
            {
                hash = hash * 59 + this.RetentionDays.GetHashCode();
            }

            return hash;
        }
    }
}

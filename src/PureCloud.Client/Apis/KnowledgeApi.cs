...
public sealed class KnowledgeApi : IKnowledgeApi
{
    ...
    #region Delete Operations (13 methods)
    ...
    public async Task<CategoryListing> GetKnowledgeKnowledgebaseLanguageCategoriesAsync(string knowledgeBaseId, string languageCode, string before = null, string after = null, string limit = null, string pageSize = null, string name = null, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrEmpty(before)) parameters.Add("before", before);
        if (!string.IsNullOrEmpty(after)) parameters.Add("after", after);
        if (!string.IsNullOrEmpty(limit)) parameters.Add("limit", limit);
        if (!string.IsNullOrEmpty(pageSize)) parameters.Add("pageSize", pageSize);
        if (!string.IsNullOrEmpty(name)) parameters.Add("name", name);

        var uri = UriHelper.GetUri($"api/v2/knowledge/knowledgebases/{knowledgeBaseId}/languages/{languageCode}/categories", parameters);
        var response = await client.GetAsync(uri, cancellationToken);
        response.EnsureSuccessStatusCode();
    ...
    public async Task<KnowledgeImport> GetKnowledgeKnowledgebaseLanguageDocumentsImportAsync(string knowledgeBaseId, string languageCode, string importId, CancellationToken cancellationToken = default)
    {
        ...
        var client = _httpClientFactory.CreateClient(PureCloudConstants.PureCloudClientName);
        var response = await client.GetAsync($"api/v2/knowledge/knowledgebases/{knowledgeBaseId}/languages/{languageCode}/documents/imports/{importId}", cancellationToken);
        response.EnsureSuccessStatusCode();
